    ...
    <div class="content">
      <div class="top">
        ...
        >
          ...
          <template v-for="(btnConfig,index) in topBtnsConfig">
            <slot
...
export default {
  ...
  computed: {
    ...
    tableColumnHandled() {
      return this.columns.filter(
        one =>
          one.isShow !== false && !["title", "slot"].includes(one.queryType)
      );
    },
    slotItems() {
    ...
    this.columns.forEach(column => {
      ...
      if (column.support) {
        ...
        if (Array.isArray(column.support)) {
          ...
          modes.forEach(mode => {
            if (column.support[mode] && column.support[mode].eventName) {
              // 编辑和新增，事件监听，并且向父组件抛出来
  ...
  methods: {
    getBtnType(btnConfig, defaultType = "primary") {
      return typeof btnConfig.btnType === "object"
        ? btnConfig.btnType.type
        : btnConfig.btnType || defaultType;
    },
    handlerUploadErr() {
    ...
    handleQueryChange(val, id) {
      this.$emit("queryChange", id, val, this.queryParams);
    ...
    generateFormatter(column) {
      column.formatter = val => {
        ...
        const target = column.options.find(
          one =>
            one[column.sourceFormat ? column.sourceFormat.value : "id"] == val
        );
        return target
          ? target[column.sourceFormat ? column.sourceFormat.label : "name"]
          : val;
      };
    }
  }
